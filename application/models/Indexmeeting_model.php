	...
	public function form_insert($data,$data1){
		...
		if($result){
			...
			$sql1 = "insert into index_meeting_participants (index_meeting_id, name, department, email, employee)
					values ('$insert_id2', '$insertname', '$insertdept', '$insertemail', '$insertemp')";
			$this->db->query($sql1);
			 }
			// echo $sql1; die;
		 ...
		 $res = $query->result(); 
		  if($res){
			  ...
			  for($i=0 ;$i<$counting ; $i++){
					...
					$query = $this->db->query("SELECT DISTINCT index_meeting.agenda_id ,index_meeting_agenda.agenda_name ,(select count(index_meeting.agenda_id) from index_meeting where index_meeting.agenda_id = $agenda_id) as counter FROM index_meeting JOIN index_meeting_agenda on index_meeting.agenda_id=index_meeting_agenda.agenda_id where index_meeting.agenda_id = $agenda_id");
					$output = $query->result();
					$result[$i] =  $output[0];
			  }
			 // print_r($result);
